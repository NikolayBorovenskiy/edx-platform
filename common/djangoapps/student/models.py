...
class UserCelebration(TimeStampedModel):
    ...
    The first_day_of_streak and last_day_of_streak fields are used to
    ...
    def __str__(self):
        return (
            '[UserCelebration] user: {}; first_day_of_streak {};  last_day_of_streak {}; longest_ever_streak {};'
        ).format(self.user.username, self.first_day_of_streak, self.last_day_of_streak, self.longest_ever_streak)
    ...
    def _calculate_streak_updates(self):
        """ Calculate the updates that should be applied to the streak fields of the provided celebration """
        first_day_of_streak = self.first_day_of_streak
        last_day_of_streak = self.last_day_of_streak
        today = self.__class__._get_now().date()  # pylint: disable=protected-access
        streak_length_to_celebrate = None

        first_ever_streak = None is first_day_of_streak is last_day_of_streak
        inside_active_streak = last_day_of_streak and first_day_of_streak
        break_length = timedelta(days=self.STREAK_BREAK_LENGTH)
        can_start_new_streak = last_day_of_streak and last_day_of_streak + break_length < today
        already_updated_streak_today = last_day_of_streak == today

        if first_ever_streak or can_start_new_streak:
            # Start new streak
            first_day_of_streak = today
            last_day_of_streak = today
        elif inside_active_streak:
            # Increment existing streak
            last_day_of_streak = today

            streak_length = (last_day_of_streak - first_day_of_streak).days + 1
            if streak_length in self.STREAK_LENGTHS_TO_CELEBRATE and not already_updated_streak_today:
                # Celebrate if we didn't already celebrate today
                streak_length_to_celebrate = streak_length
        # When the learner is in the middle of a break, we don't take any steps

        return first_day_of_streak, last_day_of_streak, streak_length_to_celebrate

    @classmethod
    def _update_streak(cls, celebration, first_day_of_streak, last_day_of_streak):
    ...
    def perform_streak_updates(cls, user, course_key):
        ...
        first_day_of_streak, last_day_of_streak, streak_length_to_celebrate = \
            celebration._calculate_streak_updates()
        # pylint: enable=protected-access

        cls._update_streak(celebration, first_day_of_streak, last_day_of_streak)
